    ...
    {{-- <div class="p-3">
        <div class="px-3 py-3 justify-between flex items-center cursor-pointer relative bg-white rounded-md"
            ...
            <div class="bg-gradient-to-r   w-6 h-6 rounded-md flex items-center justify-center mr-2 shadow-sm">
                @if (Auth('linear_user')->user()->service)
                    <img src="/images/{{ Auth('linear_user')->user()->service }}.webp" />
                @endif
            </div>
    ...
    <div class="mt-6 px-3 flex-grow">
        @php
            $linked = DB::table('linked')
                ->where('userid', Auth('linear_user')->user()->id)
                ->first();
        @endphp
        <!-- Menu items -->
        <nav class="space-y-[50px]" style="@if (!$linked) pointer-events:none; @endif">
            <a href="/invited/dash"
                class="flex  items-center justify-center px-3 py-2 text-sm font-medium text-gray-900 rounded-md {{ request()->routeIs('dash') ? 'bg-gray-100' : 'hover:bg-gray-50 hover:text-gray-900' }}">
            ...
            <div class="relative">
                <div onclick="location.href='/invited/user'"
            ...
            <div class="relative">
                <div onclick="location.href='/invited/settings'"
                    class="flex items-center justify-center px-3 py-2 text-sm font-medium text-gray-600 rounded-md hover:bg-gray-50 hover:text-gray-900 cursor-pointer">
