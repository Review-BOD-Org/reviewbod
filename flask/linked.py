...
def get_staff_info_from_email(email, db_conn):
    """Fetch staff_id, email, name, and workspace from linked_users table using email where password is not null"""
    ensure_connection()
    cursor = db_conn.cursor(pymysql.cursors.DictCursor)
    query = """
        ...
        LIMIT 1
    """
    logger.info(f"Fetching staff info for email {email}:\n{query}")
...
def get_staff_id_from_email(email, db_conn):
    """Fetch staff_id from linked_users table using email where password is not null"""
    ensure_connection()
    cursor = db_conn.cursor(pymysql.cursors.DictCursor)
    query = """
        ...
        LIMIT 1
    """
    logger.info(f"Fetching staff_id for email {email}:\n{query}")
    cursor.execute(query, (email,))
    result = cursor.fetchone()
    cursor.close()
...
def save_message_to_db(chat_id, staff_id, owner_id, sender_type, message, db_conn):
    ensure_connection()
    ...
    query = """
        ...
        VALUES (%s, %s, %s, %s, %s)
    """
    logger.info(f"Saving message to DB for chat_id {chat_id}, staff_id {staff_id}:\n{query}")
    cursor.execute(query, (chat_id, staff_id, owner_id, sender_type, message))
    db_conn.commit()
...
## STAFF INFORMATION
Current user: Name: {staff_name} And Email: ({staff_email})
Workspace: {staff_workspace}

##MORE IMPORTANT!!!!!
 - avoid stuff like this 'As your helpful data analyst assistant, I don't have access to real-time activity logs or personal task updates unless you provide specific data. Howeve'
 - always use the username for response i mean mix it with the conversation, not everytime you start with hi {staff_name}
 - refer to the staff name provided cause you have it already, so use responses with the staff info given
 = always refer to the user , because you're giving a single response, and always advice the staff 
 - mostly add template with anything data related so make it informative
 - Do not display template for empty data
 - i notice sometimes you dont give response base on data given instead you give structure of how the messahe will be, please avoid that
...
def create_new_chat(staff_id, user_query, db_conn):
    ensure_connection()
    ...
    try:
        ...
        cursor = db_conn.cursor()
        query = """
            INSERT INTO chats (staff_id, title, description, uuid)
            VALUES (%s, %s, %s, %s)
        """
        cursor.execute(query, (staff_id, title, description, chat_uuid))
        chat_id = cursor.lastrowid
        db_conn.commit()
        cursor.close()
        
        logger.info(f"Created new chat with ID {chat_id} and UUID {chat_uuid} for staff_id {staff_id}")
        ...
        query = """
            ...
            VALUES (%s, %s, %s, %s)
        """
        cursor.execute(query, (staff_id, "New Chat", "Chat conversation", chat_uuid))
        chat_id = cursor.lastrowid
        db_conn.commit()
        cursor.close()
...
# Update the WebSocket endpoint to use the new function
@app.websocket("/ws")
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if not query or not user_email or not space:
                await websocket.send_json({
                    "type": "error",
                    "message": "Query, user_id, and space are required."
                })
                continue
            
            # Get staff information from email (including id, email, name, workspace)
            staff_info = get_staff_info_from_email(user_email, conn)
            if not staff_info:
                await websocket.send_json({
                    "type": "error",
                    "message": "Invalid email or staff not found."
                })
                continue
            
            staff_id = staff_info['staff_id']
            staff_name = staff_info['name'] or "Unknown"
            staff_email = staff_info['email']
            staff_workspace = staff_info['workspace'] or "Unknown"
            ...
            if not chat_id_enc:
                new_chat_info = create_new_chat(staff_id, query, conn)
            ...
            await process_query_with_streaming(query, user_email, staff_id, owner_id, chat_id, websocket, chat_history, staff_name, staff_email, staff_workspace)
            
    except WebSocketDisconnect:
        logger.info("Client disconnected from WebSocket")

# Update the process_query_with_streaming function signature and template formatting
async def process_query_with_streaming(query: str, user_email: str, staff_id: str, owner_id: str, chat_id: str, websocket: WebSocket, chat_history: str, staff_name: str, staff_email: str, staff_workspace: str):
    # Save user message
    save_message_to_db(chat_id, staff_id, owner_id, "user", query, conn)
    ...
    logger.info(f"Processing query for user_email {user_email}, staff_id {staff_id}, owner_id {owner_id}: {query}")
    logger.info(f"Generated unique_id_with_template: {unique_id_with_template}")
    logger.info(f"User metrics loaded: {len(user_metrics)} metrics")
    logger.info(f"Staff info - Name: {staff_name}, Email: {staff_email}, Workspace: {staff_workspace}")
    ...
    if intent == "sql":
        try:            
            ...
            logger.info(f"Generated SQL Query for user_email {user_email}, staff_id {staff_id}, owner_id {owner_id}:\n{cleaned_sql}")
            ...
            logger.error(f"SQL Query Error for user_email {user_email}, staff_id {staff_id}: {str(e)}")
            await websocket.send_json({
                "type": "error", 
                "message": str(e)
            })
    ...
    try:
        ...
        message_id = insert_streaming_message(
            chat_id, staff_id, owner_id, "bot", "", unique_id_with_template, conn
        )
...
def insert_streaming_message(chat_id, staff_id, owner_id, sender_type, message, unique_id_with_template, db_conn):
    ensure_connection()
    """Insert initial message with unique_id_with_template and return message_id for updates"""
    cursor = db_conn.cursor()
    query = """
        INSERT INTO chat_messages (chat_id, staff_id, owner_id, sender_type, message, unique_id_with_template)
        VALUES (%s, %s, %s, %s, %s, %s)
    """
    cursor.execute(query, (chat_id, staff_id, owner_id, sender_type, message, unique_id_with_template))
